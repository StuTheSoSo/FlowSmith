...
- Exercise durations are editable on the phone: the planner offers minute entry and 15-second adjustments; the runner allows the same in ready, setup, and paused states. Durations have a 15-second minimum and must exceed time already taught. Runner edits affect only the active run, preserve elapsed time, persist across reloads, and immediately publish updated duration/remaining time to the watch without starting the timer. Library defaults are unchanged.
